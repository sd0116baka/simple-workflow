...
请只根据系统注入的推荐器输入 JSON，选择一个建议执行的任务，并输出执行意图 artifact 本体。
...
- 不要输出能由 `recommendedPackageId` 从任务上下文包推导出来的任务摘要字段。
- 不要输出仓库状态、启动检查结果或 `artifactType`。

输出要求：
- 只输出一个 fenced JSON 代码块，不要在代码块前后添加解释文字。
- 输出的是执行意图 artifact 本体，不是追加请求。
- 字段固定为：
  - `recommendedPackageId`: 字符串，必须等于某个 `candidateTasks[].packageId`。
