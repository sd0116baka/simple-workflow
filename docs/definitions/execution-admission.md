...
授权追加发生在任务池执行推荐器的执行意图追加请求之后，用于判断已带有执行意图的任务上下文包是否可以进入执行阶段。
...
任务推荐器已经在 `canStartWork` 为 `true` 时，基于任务池 `candidateTasks` 选择任务，并产出执行意图追加请求。
...
任务推荐器只负责在 `canStartWork` 为 `true` 时，从任务池 `candidateTasks` 中选择当前建议执行的任务，并产出执行意图追加请求。

执行意图本体不包含 `artifactType`。`artifactType` 属于追加请求，由任务推荐器在追加请求外层提供。
...
执行准入器产出的执行授权、准入拒绝、人工决策请求，都必须以追加请求形式交给任务池，由任务池执行追加。
